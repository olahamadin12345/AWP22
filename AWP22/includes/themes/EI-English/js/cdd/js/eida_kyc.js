	...
	$.ajax({
  			...
  			data: false,
				...
				success: function(response)
				{
					...
					var parsedjson = $.parseJSON(response);
					var retFlag=false;
			        //loop all keys and display the error messages dynamically.. here i is firstname,lastname etc and item is â€œerror messageâ€
			       if(parsedjson["errors"]==true)
  						{
						$.each(parsedjson, function(i, item) {
			            if($( ".error_"+i).html() != item){
...
$("#nonindeightnext").click(function() {
	window.location = "/eng/kyc/cdd/emirates-id/";
});
/*** AJAX Validation ***/
//*******************************************************************************************************
...
function IsValid_Ajax_EmirateID_Sec09(sendvar)
{
	...
	$('#emininenext').prop("disabled", true); 
	$('#emininenext').html(""); 
	$('#emininenext').addClass('section-loader');
			...
			url:"/eng/cfc/cdd/GeneralFunctions.cfc?method=CF_KYC_EmirateId_Sec09",
   			type: "POST",
				...
				success: function(response)
				{
					...
					$('#emininenext').prop("disabled", false); 
					$('#emininenext').removeClass('section-loader');
					$('#emininenext').text('Update & Proceed');
		        	...
		        	else
		        	{
						...
						if(sendvar=='eidnext')
						{
		        			window.location = "/eng/kyc/cdd/eida-update-review/";
						}
						else
						{
							return parsedjson;
		...
		$.ajax({
			...
			type: "POST",
				...
				success: function(response)
				{
					...
					var parsedjson = $.parseJSON(response);
					var retFlag=false;
			        //loop all keys and display the error messages dynamically.. here i is firstname,lastname etc and item is ï¿½error messageï¿½
			        $.each(parsedjson, function(i, item) {
			            if($( ".error_"+i).html() != item){
			        		 $( ".error_"+i).html("");
			        		 $( ".error_"+i).append(item);
...
function IsValid_Ajax_AuthenticationOTP_Sec22(){
	...
	$('#twentytwonxt').prop("disabled", true); 
	$('#twentytwonxt').html(""); 
	$('#twentytwonxt').addClass('section-loader');
		...
		$.ajax({
			...
			type: "POST",
				...
				success: function(response)
				{
					...
					$('#twentytwonxt').prop("disabled", false); 
					$('#twentytwonxt').removeClass('section-loader');
					$('#twentytwonxt').text("Next"); 
					//Here parsejson             
					var parsedjson = $.parseJSON(response);
					var retFlag=false;
			        //loop all keys and display the error messages dynamically.. here i is firstname,lastname etc and item is “error message”
			       if(parsedjson["errors"]==true)
				   {
		        	...
		        	else
		        	{
							...
							window.location.replace("/eng/kyc/cdd/emirates-id/");
	...
	$(document).on('click','.btvisaback', function(){
		...
		if(strId == "")
		{
			window.location.href="/eng/kyc/cdd/emirates-id/";
		}
		else if(strId == "1")
		{
			$('#collapse').addClass('in');
